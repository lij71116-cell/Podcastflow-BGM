...
## 2026-07-01｜移动端混音试听加载失败

- **现象**：移动端点击试听报「试听音频加载失败」；桌面 Web 双轨试听正常
- **根因**：iOS 对两路远程 stream（播客代理 + BGM）预加载不稳定，`canplay` 易失败
- **修复**：移动端改调 `POST /api/mixed-audios/preview` 生成约 60 秒混音 MP3，单 `<audio>` 播放；桌面仍用双轨实时试听

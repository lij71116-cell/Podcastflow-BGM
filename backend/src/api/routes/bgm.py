...
async def upload_bgm(
    ...
    try:
        data = await service.upload_file(session_id, file.filename or "bgm.mp3", content)
...
async def validate_bgm_url(
    ...
    service = BgmService(db, get_settings())
    try:
        data = await service.validate_and_download_url(session_id, body.source_url)
    except BgmTooLargeError as exc:
        return JSONResponse(
            status_code=400,
            content={"code": 40004, "message": str(exc), "data": None},
...
async def validate_qishui_share(
    ...
    service = BgmService(db, get_settings())
    try:
        data = await service.validate_qishui_share(session_id, body.share_url)
...
async def stream_bgm(
    ...
    service = BgmService(db, get_settings())
    try:
        path, mime = await service.get_stream_file(session_id, bgm_id)

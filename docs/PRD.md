...
见 **第 16 章 V2.0 范围定稿**。V1 MVP（M1–M4）已完成。

### V1.1 / V1.2 能力并入 V2.0

淡入淡出、重新生成、搜索筛选、播放进度记忆等原 V1.1/V1.2 项已纳入 V2.0 第 16.2 节。
...
## 16. V2.0 范围定稿（2026-06-29）

> **状态**：原型已确认（`docs/prototypes/v2/` + `.sdd/tmp/ui-design-spec.md`）  
> **部署**：继续 Vercel 前端 + Railway 后端 + Volume + SQLite（E-04）  
> **V1 匿名 Session 数据**：不合并（A-05），登录后从空库开始

### 16.1 版本目标

在 V1 核心闭环之上，完成 **邮箱账号体系（双模式登录）+ 跨端同步 + 全端 UI/H5/PWA 升级 + 音频库增强 + 混音增强（含详情页覆盖式重新生成）**。

### 16.2 纳入 V2.0

| 模块 | ID | 需求 | 验收要点 |
|------|-----|------|----------|
| A | A-01 | 注册 / 登录 | 注册：用户名 + 邮箱 + 密码 + 确认；**不做邮箱验证**；登录：用户名+密码 或 邮箱+密码 |
| A | A-02 | 登出 | 清除登录态；未登录访问受保护页跳转登录 |
| A | A-03 | 资产归属 `user_id` | 播客/BGM/组合音频/任务均绑用户；API 按用户鉴权 |
| A | A-04 | 多端同步 | 同一账号在手机/电脑/多设备共享同一音频库 |
| A | A-07 | 密码管理 | **登录后修改密码**（V2.0）；**忘记密码（邮件重置）延后至 V2.x** |
| B | B-01 | UI 改版 | 创建/库/详情/播放器按 V2 Design Spec 重建 |
| B | B-02 | 移动 H5 | 手机浏览器可完成创建、浏览、播放 |
| B | B-03 | 桌面优化 | 1440px 布局、封面取色 accent、事件轴创建页 |
| B | B-04 | 登录注册页 | `/auth` 独立页，左品牌右表单（5A） |
| B | B-05 | 顶栏账号区 | 头像首字 + 用户名下拉（改密/登出） |
| B | B-06 | PWA | manifest + 轻量 SW 静态缓存 + 移动引导条 |
| C | C-03 | 列表分页 | 音频库底部分页器 |
| C | C-04 | 播放进度记忆 | 全局播放器 + 详情内嵌播放器分别持久化进度 |
| C | C-06 | 批量删除 | 多选卡片 + 底栏确认删除 |
| D | D-01 | 淡入淡出 | MixConfig 增加 fade_in / fade_out（秒） |
| D | D-02 | 重新生成 | 详情页展开预填原配置 → 覆盖合成当前资产 |

### 16.3 V2.0 明确不做

| ID | 内容 |
|----|------|
| A-05 | V1 匿名 Session 数据合并 |
| A-06 | 手机号 / 微信登录 |
| — | 邮箱注册验证 |
| — | BGM 模板库 / 播客链接库（规划 V3） |
| — | OSS/S3 / Postgres 迁移 |
...
| 编号 | 规则 |
|------|------|
| BR-07-V2 | 资产按 **`user_id`** 隔离（替代 V1 `session_id`） |
| BR-10 | 未登录不可访问创建/音频库/详情等核心页 |
| BR-11 | 播放进度按 `(user_id, mixed_audio_id, player_context)` 持久化；`player_context` 为 `global` 或 `inline` |
| BR-12 | 重新生成 **覆盖** 原 mixed 文件与元数据，不新建资产 ID |
| BR-13 | 批量删除须二次确认；删除正在播放项时停止播放器 |
| BR-14 | 淡入淡出默认 0 秒；上限 30 秒 |

### 16.5 V2 页面与原型

| 页面 | 路由 | V2 原型 |
|------|------|---------|
| P-Auth | `/auth` | `docs/prototypes/v2/auth.html` |
| P01 创建 | `/` | `docs/prototypes/v2/index.html`（3 Tab 事件轴） |
| P02 音频库 | `/library` | `docs/prototypes/v2/library.html` |
| P03 详情 | `/detail/:id` | `docs/prototypes/v2/detail.html` |
| G01 播放器 | 全局 | Design Spec §3.2 |
| PWA | — | manifest + 引导条（原型示意） |

Design Spec：`.sdd/tmp/ui-design-spec.md`

### 16.6 V2 外部依赖（A5-3）

| 服务 | 用途 | 配置字段 | V2 必需 | 缺失策略 |
|------|------|----------|---------|----------|
| 邮件发送 | 忘记密码重置链接 | `RESEND_API_KEY` 等 | **否（V2.x）** | V2.0 不做 forgot-password |
| FFmpeg + ffprobe | 合成（含 fade） | 同 V1 | 是 | 同 V1 |
| 小宇宙公开页 | 解析 | 同 V1 | 是 | 同 V1 |
| SQLite + 本地存储 | 数据与音频 | 同 V1 | 是 | — |

### 16.7 V2 里程碑

| 里程碑 | 交付物 |
|--------|--------|
| M5 V2 Mock + Auth UI | V2 页面 Mock + 登录注册页 |
| M6 账号后端 | User 表 + JWT/Cookie 鉴权 + 资产迁移 user_id |
| M7 V2 功能闭环 | 分页/进度/批量删/淡入淡出/重新生成 |
| M8 V2 E2E | 登录 → 创建 → 多端同步 → PWA 冒烟 |
